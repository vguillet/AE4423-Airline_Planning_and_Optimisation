...
ac_dict, airports_dict, distances_df, routes_dict, traffic_df = generate_data()
...
for aircraft_ref, aircraft in self.network.ac_dict.items():
    ...
    edges_df = edges_df.reindex(index=list(node for node in self.network.airports_dict.keys()),
                                fill_value=0)
